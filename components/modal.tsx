...
const Modal: React.FC<ModalProps> = ({ isOpen, onChange, title, description, children }) => {
  ...
  return <Dialog.Root open={isOpen} defaultOpen={isOpen} onOpenChange={onChange}>
    <Dialog.Portal>
      <Dialog.Overlay className="fixed inset-0 bg-neutral-900/90" />
      <Dialog.Content className="fixed drop-shadow-md border border-neutral-700 top-[50%] left-[50%] max-h-full h-full md:h-auto md:max-h-[85vh] w-full md:w-[90vw] md:max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-md bg-neutral-800 p-[25px] focus:outline-none">
        <Dialog.Title className="text-xl font-bold text-center mb-4">{ title }</Dialog.Title>
        <Dialog.Description className="text-sm text-center mb-4">{ description }</Dialog.Description>
        <div>{ children }</div>
        <Dialog.Close className="absolute top-[10px] right-[10px] text-neutral-400 hover:text-white" onClick={() => onChange(false)}>
          <IoMdClose />
        </Dialog.Close>
      </Dialog.Content>
    </Dialog.Portal>
